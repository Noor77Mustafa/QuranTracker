...
export async function getVerses(
  ...
  try {
    // Make sure we get the Arabic text in Uthmani script and add fields parameter to ensure text is included
    let url = `${QURAN_COM_API_URL}/verses/by_chapter/${surahId}?language=en&page=${page}&limit=${limit}&text_type=uthmani&fields=text_uthmani`;
    ...
    // Log the first verse for debugging
    if (data.verses && data.verses.length > 0) {
      console.log(`First verse of Surah ${surahId}:`, JSON.stringify(data.verses[0]));
    }
    
    // Return the verses without any processing - the API is already providing the correct text
    return data.verses || [];
  } catch (error) {
